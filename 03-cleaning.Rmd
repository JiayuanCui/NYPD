...
crime %>% dplyr::select(CMPLNT_NUM,ExactDate)
```
...
crime %>% dplyr::select(OFNS_DESC,CrimeType,ExactDate)
```
...
crime %>% dplyr::select(CrimeType,ExactDate,ExactTime,day,month,Time)
```